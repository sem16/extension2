...
export default class ExtensionCommandSet extends BaseListViewCommandSet<
  IExtensionCommandSetProperties
> {
  private data: {}[];
    ...
    if (!this.hideAll) {
      switch (event.itemId) {
        case "COMMAND_1":
          ...
          break;
        case "COMMAND_2":
          const dialogPlaceHolder = document.body.appendChild(
          ...
          try {
            console.log(
              window["$ic"].states[0].itemMap["items"][4].listFacet_385.title
            );
          } catch (e) {}
